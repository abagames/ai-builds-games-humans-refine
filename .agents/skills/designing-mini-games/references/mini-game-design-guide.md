...
- [ ] Does the document contain the Causal Chain Audit (Appendix A 0.6) and Degenerate Strategy Declaration (Appendix A 0.7) as filled tables, not prose claims?
...
## 0.6 Causal Chain Audit

Trace every scoring and death/penalty event to an in-world cause. This is a mandatory filled table, not a yes/no check — producing the rows forces retrieval of causality-design failures that a prose self-assertion skips. It is normally written after §1–§2 are drafted; keep the `0.6` label so workflows can locate it.

| Event | Immediate in-world cause | Chain back to player intent | Raw-input shortcut? |
| :-- | :-- | :-- | :-- |
| <score/death event> | <the object/state overlap or collapse that fires it> | <event ← world state ← earlier player action> | <No — why input alone cannot fire it> |

Include one row per scoring rule and one per death/penalty cause. A "Yes" in the last column is a design defect: the event rewards or punishes raw input facts (or non-action) instead of in-world causality — redesign before continuing.

## 0.7 Degenerate Strategy Declaration

Enumerate the degenerate strategies the chosen input scheme allows and why each loses. Cover at least the scheme's monotonous policy set (idle and mashing always; hold-only for one-button; single-button-spam and hold-everything for multi-button), plus any camping or stalling strategy the rules invite. Like 0.6, this is a mandatory filled table; keep the `0.7` label.

| Strategy | What happens | Why it is not optimal |
| :-- | :-- | :-- |
| <idle / mash / hold / camp …> | <what the world does in response> | <in-world reason it scores worse than skilled play — never a meta penalty> |

Close with the expected exploratory-ratio mechanism: one sentence on why skilled play should outscore the best monotonous policy.
